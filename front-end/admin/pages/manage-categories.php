...
<main class="main cont">
  <form method="POST">
    <?php for ($i = 0; $i < count($categories); $i++): $c = $categories[$i]; ?>
      <section data-id="<?= $c['id'] ?>" data-order-id="<?= $c['order_id'] ?>" class="js-editable-entity editable-entity">
        <div class="editable-entity__name-and-btns">
          <h3 class="js-category-name category-name <?= $c['hidden'] ? 'category-name--hidden' : '' ?> editable-entity__name"><span class="category-name__num">#<?= $c['id'] ?></span> Категория "<?= $c['name'] ?>" <?= $c['hidden'] ? '(скрыта)' : '' ?></h3>
          <div class="editable-entity__btns">
            ...
            <?php if ($c['hidden']): ?>
              <button class="js-unhide-btn btn bs-like-btn bs-like-btn--info" title="<?= $c['hidden'] ? 'Вернуть на сайт' : 'Скрыть с сайта' ?>" type="button">
            ...
            <?php endif ?>
            <button class="js-delete-btn btn bs-like-btn bs-like-btn--danger" title="Удалить" type="button">
        ...
        <section class="js-editable-entity__subsection js-dn editable-entity__subsection">
          ...
          <p class="db mb-1em">
            Скрыть категорию?&nbsp;
            <label style="font-size: .8em;">Нет <input type="radio" name="<?= $c['id'] ?>[hidden]" value="0" <?= $c['hidden'] ? '' : 'checked' ?>></label>
            <label style="font-size: .8em;">Да <input type="radio" name="<?= $c['id'] ?>[hidden]" value="1" <?= $c['hidden'] ? 'checked' : '' ?>></label>
          </p>
          <label class="db mb-1em">
            <p>Описание на главной странице в слайдере:</p>
            <textarea class="textarea textarea--full textarea--w-code" name="<?= $c['id'] ?>[description]"><?= $c['description'] ?></textarea>
          </label>
          <label class="db mb-1em">
            <p>Перенос строк в названии категории (если нужно):</p>
            <textarea class="textarea textarea--small textarea--w-code" name="<?= $c['id'] ?>[name_view]"><?= $c['name_view'] ?></textarea>
          </label>
          <div class="mb-1em">
            Поменять картинку:
            ...
            <div class="editable-entity__imgs-grid">
              ...
              <div>
                ...
                <label for="<?= $c['id'] ?>-changeCatImg" class="btn btn--admin" style="font-size: .8em; margin-bottom: .5em; display: inline-block;">Поменять картинку категории</label>
                <input class="js-changeCatImg" type="file" id="<?= $c['id'] ?>-changeCatImg" name="<?= $c['id'] ?>[img]" accept="image/png, image/jpeg">
                <div style="margin-bottom: 55px;" class="js-changeCatImgHolder img-holder img-holder--smaller"></div>
              </div>
            </div>
          </div>
        </section>
